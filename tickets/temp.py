...
def pop_next_ticket():
    for item in service_urls:
        if len(line_of_cars[item]) != 0:
            return line_of_cars[item].popleft()
    return 0  # if queue is empty
##########################
...
class Processing(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        return context
# Show next ticket; processing next ticket
...
<div> lala </div>
   <!-- {% if {{ticket}} %}Next ticket {{ticket}}
    {% else %}Waiting for the next client
    {% endif %}
     Next ticket {{ticket}} -->
      <!--  Test
    </div> -->
